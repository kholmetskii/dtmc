...
def benchmark_environment(
    entry: dict[str, Any], data_root: Path = DATA_ROOT
) -> dict[str, str]:
...
def manifest_entries(mode: str, data_root: Path = DATA_ROOT) -> list[dict[str, Any]]:
...
def record_environment(
    ...
    benchmark_scope: list[str] | None = None,
) -> None:
    results_root.mkdir(parents=True, exist_ok=True)
    environment = os.environ.copy()
    environment.update(THREAD_ENV)
    ...
    record = {
        ...
        "dataset_manifest_sha256": digest(data_root / "manifest.json"),
        "git_commit": capture(["git", "rev-parse", "HEAD"]),
        "git_status": capture(["git", "status", "--short"]),
        "thread_environment": THREAD_ENV,
        "benchmark_scope": benchmark_scope or "complete suite",
    }
...
def analyse(
    results_root: Path = RESULTS_ROOT, data_root: Path = DATA_ROOT
) -> None:
    run(
        [
            str(python_executable()),
            str(BENCHMARK_ROOT / "analyse.py"),
            "--data-root",
            str(data_root),
            "--results-root",
            str(results_root),
